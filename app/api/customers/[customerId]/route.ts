...
import { notifyCustomerDeleted } from "@/lib/websocket-server";
...
export async function DELETE(request: Request, { params }: { params: { customerId: string } }) {
  try {
    ...
    try {
      notifyCustomerDeleted(customerId.toString()); // Convert to string
      console.log("[v0] Real-time notification sent for customer deletion");
    } catch (wsError) {
