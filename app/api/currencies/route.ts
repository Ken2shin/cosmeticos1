...
export async function GET() {
  try {
    ...
    try {
      ...
      console.log("[v0] Currencies API: Database unavailable, using fallback currencies")
      currencies = []
    }
    ...
    if (!currencies || currencies.length === 0) {
      const fallbackCurrencies = [
        ...
        { code: "NIO", name: "Córdoba Nicaragüense", symbol: "C$", flag_emoji: "🇳🇮" },
      ]
    ...
    console.log("[v0] Currencies API: Returning emergency fallback currencies")
    return NextResponse.json(fallbackCurrencies, { status: 200 })
  }
}
