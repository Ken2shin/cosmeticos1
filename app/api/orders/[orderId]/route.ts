...
import { notifyOrderDeleted } from "@/lib/sse-notifications"
...
export async function DELETE(request: Request, { params }: { params: { orderId: string } }) {
  try {
    ...
    try {
      notifyOrderDeleted(orderId.toString())
      console.log("[v0] Real-time SSE notification sent for order deletion")
    } catch (sseError) {
